...
> **Purpose:** Tests the `.native` CSS class — the foundation styling applied to all notes in the vault. No module colors. Compare with `benchmark-bare` (no styles) and `benchmark-native-module` (with module variables).
...
`.native` styles headers, paragraphs, bold, italic, images, blockquotes, callouts, metadata, MetaBind inputs, and buttons. **No color theming** — only structural/layout base.

Compare with:
- `benchmark-bare` — no styles at all
- `benchmark-native-module` — adds module CSS variables
- `benchmark-module-*` — full per-module theming
