...
# Benchmark: Inthima Module

> **Purpose:** Tests the Inthima module theming (`native` + `module` + `inthima`). Inthima (`Φ`) is the deepest, most intimate module — personal beliefs and core values (Axioms). Compare colors with `benchmark-module-02-memorium` and `benchmark-module-01-cognitio`.
...
```yaml
cssclasses: [native, module, inthima]
tags: [benchmark]
```
...
The `inthima` class sets `--highlight` to the Inthima accent color via `am-class-native-module-00-inthima.css`. Inthima is the first module, representing the deepest layer of the self. Compare colors across modules:
- `benchmark-module-02-memorium` — Memorium accent
- `benchmark-module-01-cognitio` — Cognitio accent
