...
# Benchmark: Memorium Module

> **Purpose:** Tests the full Memorium module theming (`native` + `module` + `memorium`). Memorium (`Ψ`) is the journaling module. Compare colors with `benchmark-module-01-cognitio` and `benchmark-module-00-inthima`.
...
The `memorium` class sets `--highlight` to the Memorium accent color via `am-class-native-module-02-memorium.css`. All Obsidian UI elements (links, tags, callout borders, checkboxes, code backgrounds, MetaBind inputs) are tinted with this color.

Compare with:
- `benchmark-native` — no module theme, just structural base
- `benchmark-native-module` — module variables without specific color
- `benchmark-module-01-cognitio` — different accent color
- `benchmark-module-00-inthima` — different accent color
