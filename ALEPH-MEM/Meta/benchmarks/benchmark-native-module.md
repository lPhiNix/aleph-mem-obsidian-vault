...
> **Purpose:** Tests `.native` + `.module` together. The `.module` class sets shared CSS variables (`--highlight`, `--primary`, `--secondary`, `--dark`) without a specific module color. Useful as a baseline before adding per-module theming.
...
`.module` adds CSS custom properties for theming (`--highlight`, `--primary`, `--secondary`, `--dark`) that Control:
- Link colors
- Bold/italic/emphasis colors
- Highlight colors
- Tag pills
- Callout borders, titles, and icons
- Table borders
- Blockquote borders
- Checkboxes
- MetaBind inputs and buttons
- Dataview table headers and errors
- Collapse icons and metadata properties

Without a per-module class, these use fallback values. Compare with `benchmark-module-*` to see the difference a module-specific color makes.
