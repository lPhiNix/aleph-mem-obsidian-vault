...
# Benchmark: Viventia Module

> **Purpose:** Tests the Viventia module theming (`native` + `module` + `viventia`). Viventia (`Z`) is the vitality module — hobbies, life, fun, passion, creativity. Compare colors with other modules.
...
```yaml
cssclasses: [native, module, viventia]
tags: [benchmark]
```
...
The `viventia` class sets `--highlight` to the Viventia accent color via `am-class-native-module-05-viventia.css`. Viventia is the vitality and hobbies module — tracking activities, practice sessions, and creative pursuits.
