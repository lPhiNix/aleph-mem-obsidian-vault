...
# Benchmark: Devs Module

> **Purpose:** Tests the Devs module theming (`native` + `module` + `devs`). Devs (`Λ`) is the creation module — projects, bitácoras, roadmaps, decisions, postmortems. Compare colors with other modules.
...
The `devs` class sets `--highlight` to the Devs accent color via `am-class-native-module-04-devs.css`. Devs is the creation and projects module — it integrates with Ordus for per-project kanban boards.
