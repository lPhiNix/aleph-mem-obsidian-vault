...
# Benchmark: Ordus Module

> **Purpose:** Tests the Ordus module theming (`native` + `module` + `ordus`). Ordus (`Π`) is the order and task management module — kanban boards, tasks, and subtasks. Compare colors with other modules.
...
The `ordus` class sets `--highlight` to the Ordus accent color via `am-class-native-module-07-ordus.css`. Ordus is the task management module — the most complex interactive system in the vault.
...
- `benchmark-ordus-checklist` — Ordus colors + full interactive checklist
