...
> **Purpose:** Baseline for Obsidian's default Markdown rendering. No `cssclasses`, no custom frontmatter, no plugin styling applied. Compare against all other benchmarks.
...
## Frontmatter Display (YAML)

```yaml
tags: [benchmark]
```

> **End of bare benchmark.** Compare with `benchmark-native`, `benchmark-module-*`, and `benchmark-utilities`.
